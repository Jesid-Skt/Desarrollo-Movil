...
export default function StartScreen({ navigation }) {
  ...
  const { isTablet, horizontalPadding, titleSize, maxContentWidth } =
    useResponsive();

  const clasesFiltradas = CLASES.filter((clase) => {
    const coincideNivel = nivel === "Todos" || clase.nivel === nivel;
    const texto =
      `${clase.titulo} ${clase.nivel} ${clase.profesor.nombre}`.toLowerCase();
  ...
  return (
    ...
    >
      ...
      <Text style={styles.descripcion}>
        Practica inglés con profesores que se adaptan a tus objetivos.
      </Text>
      ...
      <View style={[styles.listaClases, isTablet && styles.listaTablet]}>
        {clasesFiltradas.map((clase) => (
          <View key={clase.id} style={isTablet ? styles.itemTablet : undefined}>
            <Card
              clase={clase}
              onPress={() =>
                navigation.navigate("DetalleClaseScreen", {
                  claseId: clase.id,
                })
              }
            />
